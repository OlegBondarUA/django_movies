    ...
    while True:
        ...
        for index, (metadata, thumbnail, original) in enumerate(
            ...
            with requests.Session() as session:
                img_response = session.get(google_images['original'], timeout=TIME_OUT)
            image_name = f'{film}.jpg'.replace(' ', '-')

            with open(f'media/background/{image_name}', 'wb') as file:
                file.write(img_response.content)

            film.background = f'/Users/olegbondar/Python/project_django/Beetroot_django/media/background/{image_name}'
            film.save()
